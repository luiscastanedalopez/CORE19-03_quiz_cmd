...
describe("CORE19-03_quiz_cmd", function () {
    ...
    it('', async function () {
        ...
        if (error_critical) {
            ...
            if (!fs.existsSync(quizzes_back)) {
                [_, path_ok] = await to(fs.move(quizzes_orig,quizzes_back, {"overwrite": true}));
            }
            [error_deps, path_ok] = await to(fs.move(quizzes_test,quizzes_orig, {"overwrite": true}));
            if (error_deps) {
    ...
    after("Restoring the original file", async function () {
        if (fs.existsSync(quizzes_back)){
            [_, _] = await to(fs.move(quizzes_back, quizzes_orig, {"overwrite": true}));
        } else {
            [_, _] = await to(fs.move(quizzes_test, quizzes_orig, {"overwrite": true}));
        }
    });
});